...
function smoothDampFloat(current, target, velRef, smoothTime, maxSpeed, deltaTime) {
  ...
  if (origMinusCurrent * outMinusOrig > 0) { out = originalTo; velRef.v = 0; }
...
function centroid(points) {
  let x = 0, y = 0; const n = points.length || 1;
...
export const GridScan = ({
  ...
  const bufX = useRef([]); const bufY = useRef([]); const bufT = useRef([]); const bufYaw = useRef([]);
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      try {
        ...
        console.warn('[GridScan] Post-processing unavailable:', err);
        composer = null;
  ...
  useEffect(() => {
        ...
        if (ts - lastDetect >= 33) {
          ...
          try {
            ...
            if (res && res.detection) {
              const det = res.detection; const box = det.box;
              const vw = video.videoWidth || 1; const vh = video.videoHeight || 1;
              const cx = box.x + box.width * 0.5; const cy = box.y + box.height * 0.5;
              const nx = (cx / vw) * 2 - 1; const ny = (cy / vh) * 2 - 1;
              medianPush(bufX.current, nx, 5); medianPush(bufY.current, ny, 5);
              const nxm = median(bufX.current); const nym = median(bufY.current);
              ...
              const leftEye = res.landmarks.getLeftEye(); const rightEye = res.landmarks.getRightEye();
              const lc = centroid(leftEye); const rc = centroid(rightEye);
              const tilt = Math.atan2(rc.y - lc.y, rc.x - lc.x);
              medianPush(bufT.current, tilt, 5); tiltTarget.current = median(bufT.current);
              ...
              const leftCheek = jaw[3] || jaw[2]; const rightCheek = jaw[13] || jaw[14];
              const dL = dist2(tip, leftCheek); const dR = dist2(tip, rightCheek);
              ...
              medianPush(bufYaw.current, yawSignal, 5); yawTarget.current = median(bufYaw.current);
    ...
    return () => {
      stop = true;
      if (video) { const stream = video.srcObject; if (stream) stream.getTracks().forEach(t => t.stop()); video.pause(); video.srcObject = null; }
    };
